...
X.loc[:, 'Close'] = (np.log(X['Close']) - np.log(X['Close'].shift(1))).round(4);    
X = X[X['Close'].notna()];
...
Y.loc[:, 'Close']  = (np.log(Y['Close']) - np.log(Y['Close'].shift(1))).round(4) 
Y = Y[Y['Close'].notna()];
...
print(f"\n Mutual Information: {mutualInformation(XY['Close_x'], XY['Close_y'], 4) : .4f}");
